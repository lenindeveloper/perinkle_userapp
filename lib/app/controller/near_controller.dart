  ...
  Future<void> getHomeData() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var element in _salonList) {
        markers.add(Marker(
          markerId: MarkerId(element.id.toString() + 'salon'),
      ...
      for (var element in _individualList) {
        markers.add(Marker(
          markerId: MarkerId(element.uid.toString() + 'freelancer'),
          position: LatLng(element.lat as double,
              element.lng as double), //position of marker
          infoWindow: InfoWindow(
            title: element.userInfo!.firstName.toString() +
                ' ' +
                element.userInfo!.lastName.toString(),
          ),
          icon: BitmapDescriptor.defaultMarker, //Icon for Marker
        ));
      }
